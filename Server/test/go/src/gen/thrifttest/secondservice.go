...
func NewSecondServiceProcessor(handler SecondService) *SecondServiceProcessor {
	...
	self167 := &SecondServiceProcessor{handler: handler, processorMap: make(map[string]thrift.TProcessorFunction)}
	self167.processorMap["blahBlah"] = &secondServiceProcessorBlahBlah{handler: handler}
	self167.processorMap["secondtestString"] = &secondServiceProcessorSecondtestString{handler: handler}
	return self167
}
